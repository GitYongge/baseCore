...
import com.costech.entity.system.Menu
...
class Jurisdiction {
     ...
     * @return
     */
    static boolean hasJurisdiction(String menuUrl){
    ...
    static boolean readMenu(List<Menu> menuList,String menuUrl,Session session,String USERNAME){
        for(int i=0;i<menuList.size();i++){
            if(menuList.get(i).MENU_URL.split(".do")[0] == menuUrl.split(".do")[0]){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
                if(!menuList.get(i).hasMenu){				//判断有无此菜单权限
                    return false
                }else{											//按钮判断
                ...
                if(!readMenu(menuList.get(i).subMenu,menuUrl,session,USERNAME)){//继续排查其子菜单
                    return false
                }
            }
        }
     ...
     * @return
     */
    static boolean buttonJurisdiction(String menuUrl, String type){
        //判断是否拥有当前点击菜单的权限（内部过滤,防止通过url进入跳过菜单权限）
        /**
         * 根据点击的菜单的xxx.do去菜单中的URL去匹配，当匹配到了此菜单，判断是否有此菜单的权限，没有的话跳转到404页面
         * 根据按钮权限，授权按钮(当前点的菜单和角色中各按钮的权限匹对)
     ...
     * @return
     */
    static boolean readMenuButton(List<Menu> menuList,String menuUrl,Session session,String USERNAME, String type){
        for(int i=0;i<menuList.size();i++){
            if(menuList.get(i).MENU_URL.split(".do")[0] == menuUrl.split(".do")[0]){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
                if(!menuList.get(i).hasMenu){				//判断有无此菜单权限
                    return false
                    ...
                    Map<String, String> map = (Map<String, String>)session.getAttribute(USERNAME + Const.SESSION_QX);//主职角色按钮权限(增删改查)
                    Map<String, List<String>> maps = (Map<String, List<String>>)session.getAttribute(USERNAME + Const.SESSION_QX2);//副职角色按钮权限(增删改查)
                    String MENU_ID =  menuList.get(i)MENU_ID
                    if("admin"== USERNAME){
                        return true
                    }else{
                        Boolean badd = false
                ...
                if(!readMenuButton(menuList.get(i).subMenu,menuUrl,session,USERNAME,type)){//继续排查其子菜单
                    return false
                }
            }
        }
     ...
     * @return
     */
    static String getDEPARTMENT_IDS(){
     ...
     * @return
     */
    static String getDEPARTMENT_ID(){
     ...
     */
    @SuppressWarnings("unchecked")
    static Map<String, String> getHC(){
