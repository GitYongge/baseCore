package com.costech.plugin.websocketInstantMsg
...
class ChatServer extends WebSocketServer {
    ...
    ChatServer(InetSocketAddress address) {
        super(address)
    }
     ...
     * @throws Exception
     */
    void usersendmsg(String message, WebSocket conn){
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()				//普通类从spring容器中拿出service
        HismsgService hismsgService = (HismsgService)webctx.getBean("hismsgService")
        ...
        if("friend"==type){
            ...
            IQgroupService iqgroupService = (IQgroupService)webctx.getBean("iqgroupService")
            PageData pd = new PageData()
            pd.put("QGROUP_ID", id)
            ...
            try {
                ...
                for(int i=0;i<varList.size();i++){
                    ...
                    if(user==USERNAME)continue													//不发给本人(群消息的发送者不接收自己发的消息)
     ...
     * @throws Exception
     */
    void userjoin(String user, WebSocket conn){
        ChatServerPool.addUser(user,conn)												//向连接池添加当前的连接对象
        PageData pd = new PageData()
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()		//普通类从spring容器中拿出service
        ImstateService imstateService = (ImstateService)webctx.getBean("imstateService")
        pd.put(Const.SESSION_USERNAME,user)
        try {
            PageData ipd =imstateService.findByUsername(pd)
            if(null == ipd || "online" == ipd.getString("ONLINE")){						//只有用户设置的状态为在线的时候，才会向别人发送上线消息
                JSONObject result = new JSONObject()
                result.element("type", "user_join")
                result.element("user", user)
                FriendsService friendsService = (FriendsService)webctx.getBean("friendsService")
     ...
     * @throws Exception
     */
    void useronline(String user, WebSocket conn){
        PageData pd = new PageData()
        pd.put(Const.SESSION_USERNAME,user)
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()		//普通类从spring容器中拿出service
        try {
     ...
     * @throws Exception
     */
    void userhide(String user, WebSocket conn){
        PageData pd = new PageData()
        pd.put(Const.SESSION_USERNAME,user)
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()		//普通类从spring容器中拿出service
        try {
     ...
     * @throws Exception
     */
    void applyfriend(String message, WebSocket conn){
        JSONObject result = new JSONObject()
     ...
     * @throws Exception
     */
    void userremove(String message, WebSocket conn){
        String [] arrmessage = message.split(",fh,")
        JSONObject result = new JSONObject()
     ...
     * @throws Exception
     */
    void agreefriend(String message, WebSocket conn){
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()		//普通类从spring容器中拿出service
     ...
     * @throws Exception
     */
    void applyqgroup(String message, WebSocket conn){
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()		//普通类从spring容器中拿出service
        QgroupService qgroupService = (QgroupService)webctx.getBean("qgroupService")
        PageData pd = new PageData()
     ...
     * @throws Exception
     */
    void agreeqgroup(String message, WebSocket conn){
        String [] arrmessage = message.split(",fh,")
     ...
     * @throws Exception
     */
    void kickoutqgroup(String message, WebSocket conn){
        String [] arrmessage = message.split(",fh,")
        String USERNAME = arrmessage[0]	//用户名
        String QGROUP_ID = arrmessage[1]	//群ID
        JSONObject result = new JSONObject()
     ...
     * @throws Exception
     */
    void delqgroup(String QGROUP_ID, WebSocket conn){
        PageData pd = new PageData()
        pd.put("QGROUP_ID", QGROUP_ID)
        WebApplicationContext webctx=ContextLoader.getCurrentWebApplicationContext()		//普通类从spring容器中拿出service
        try {
            ...
            QgroupService qgroupService = (QgroupService)webctx.getBean("qgroupService")
            SysmsgService sysmsgService = (SysmsgService)webctx.getBean("sysmsgService")
            IQgroupService iqgroupService = (IQgroupService)webctx.getBean("iqgroupService")
    ...
    void onFragment(WebSocket conn, Framedata fragment ) {
    }
