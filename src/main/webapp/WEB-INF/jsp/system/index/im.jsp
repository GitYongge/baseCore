...
layui.config({
	  base: 'plugins/im/dist/js/' //扩展 JS 所在目录
	}).extend({
		readyMenu: 'readyMenu'
	});
...
layui.use(['layim','readyMenu'], function(layim){
  ...
  layim.on('sendMessage', function(data){
	 ...
	 *	mine.content：消息内容
	 */
	 websocket.send(",fhadmin-msg,"+user+",costech,"+uname+",costech,"+To.id+",costech,"+To.type+",costech,"+mine.avatar+",costech,"+mine.content);
  });
...
function removeIFromFriend(FUSERNAME){
	websocket.send(',fhadmin-remove,'+user+',costech,'+FUSERNAME);
}
...
function agreeFriend(FUSERNAME){
	websocket.send(',fhadmin-agreefriend,'+user+",costech,"+FUSERNAME);
}

//同意对方申请加群，发送同意信息给对方
function agreeQgroup(uid,group){
	websocket.send(',fhadmin-agreeqgroup,'+uid+",costech,"+group);
}

//踢出群，发踢出信息给对方
function kickoutQgroup(uid,group){
	websocket.send(',fhadmin-kickoutqgroup,'+uid+",costech,"+group);
}
